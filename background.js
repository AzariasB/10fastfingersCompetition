...
var connected = false;
var websiteUrl = 'http://10fastfingers.com/';
var lastCompetition;
var nwCompetitions = 0;
var languageTestVal = 'english';
var languageTextName = 'English';
var refreshTimeout = 10;
var valRegex = /\s*var\s+competitions_participated\s*=\s*\[(\"\d+\",)*(\"\d+\")?\];/;
var scripReg = new RegExp('<script[\\s\\S\\d\\D]*?>[\\s\\S]*?</script>', 'g');
var linkReg = new RegExp('<link[\\d\\D]*?>', 'g');
var competParticipated;
var currentTimeout;
...
function getCompetitionURl(competRef) {
    return websiteUrl + competRef;
}

function getTypingTestUrl() {
    return websiteUrl + 'typing-test/' + languageTestVal;
}
...
function click() {
    refresh(function () {
        if (connected) {
            if (nwCompetitions > 0) {
                openCompetition(lastCompetition);
                refreshCompetitions(1.2); //Refresh competition after 2 minutes of going in
            } else {
                openFastFingers();
            }
        }
...
function lookForNewCompetitions() {
    if (HTMLinXHR()) {
        ...
        xhr.open("GET", websiteUrl + "competitions", true);
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                // innerText does not let the attacker inject HTML elements.
                var res = valRegex.exec(xhr.responseText);
                eval(res[0]);//Get "competition_participated" var.
                competParticipated = competitions_participated;
...
function processCore(tbody) {
    ...
    if (nwCompetitions > 0 && lastCompetition) {
        chrome.browserAction.setTitle({title: nwCompetitions === 1 ? 'There is a new competition' :
    ...
    chrome.browserAction.setBadgeText({text: ("" + nwCompetitions) || ("" + 0)});
    refreshCompetitions(refreshTimeout);//Refresh to see if there is a new competition every 10 minutes
}
 ...
 */
function refreshCompetitions(timeout/*in minutes*/) {
...
function HTMLinXHR() {
    ...
    req.open('GET', websiteUrl + 'competitions', false);
    try {
...
function openCompetition(competitionId) {
    chrome.tabs.create({url: getCompetitionURl(competitionId)});
}
...
function listenToStorage() {
    chrome.storage.onChanged.addListener(function (items) {
        languageTestVal = (items.favLangVal ? items.favLangVal.newValue : languageTestVal);
        languageTextName = (items.favLangName ? items.favLangName.newValue : languageTextName);
        refreshTimeout = (items.refreshTimeout ? items.refreshTimeout.newValue : refreshTimeout);
        refresh();
    });
}
